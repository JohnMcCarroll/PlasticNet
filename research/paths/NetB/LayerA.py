...
class LayerA(nn.Module):
    def __init__(self, block):
        ...
        self.block1 = block[0]
        self.connectionRange = 1
        if len(block) > 1:
            self.block2 = block[1]
            self.block3 = block[2]
            self.connectionRange = 3
    ...
    def prune(self, threshold):
        ...
        for key in list(self.scalars.keys()):
            # apply crude weight decay
            self.scalars[key] = nn.parameter.Parameter(self.scalars[key] * 0.99)
    ...
    def wire(self, numNew, upstreamList):
        # attempts to make designated amount of connections - no guarentee
        while numNew > 0:
            numNew -= 1

            upstream = random.choice(upstreamList)
            downstream = random.randint(1, self.connectionRange)                                           # might want to not hardcode?
    ...
    def baselineWire(self, position):
        ...
        if position == 4:
            ...
            self.connections['1'].append(17)
            self.connections['1'].append(18)
            self.scalars['116'] = nn.parameter.Parameter(torch.Tensor([random.random()]))
            self.scalars['117'] = nn.parameter.Parameter(torch.Tensor([random.random()]))
            self.scalars['118'] = nn.parameter.Parameter(torch.Tensor([random.random()]))
        if position == 5:
