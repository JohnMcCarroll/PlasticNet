...
class LayerA(nn.Module):
    ...
    def baselineWire(self, position):
        ...
        if position == 4:
            ...
            self.connections['1'].append(8)
            self.connections['1'].append(9)
            self.scalars['17'] = nn.parameter.Parameter(torch.Tensor([random.random()]))
            self.scalars['18'] = nn.parameter.Parameter(torch.Tensor([random.random()]))
            self.scalars['19'] = nn.parameter.Parameter(torch.Tensor([random.random()]))
        # if position == 5:
